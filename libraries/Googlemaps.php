...
/**
 * CodeIgniter Google Maps API V3 Class | Library Updated For Module : 21-09-2012
 *
...
class Googlemaps {
    ...
    function create_map() {
        ...
        if ($this->minifyJS) {
            self::$ci->load->library('jsmin');
            $this->output_js_contents = self::$ci->jsmin->min($this->output_js_contents);
        }
    ...
    function get_lat_long_from_address($address) {
        ...
        if ($this->geocodeCaching) { // if caching of geocode requests is activated
            ...
            self::$ci->db->select("latitude,longitude, formatted_address");
            self::$ci->db->from("geocoding");
            self::$ci->db->where("address", trim(strtolower($address)));
        ...
        if ($data->status == "OK") {
            ...
            if ($this->geocodeCaching) { // if we to need to cache this result
                ...
                self::$ci->db->insert("geocoding", $data);
            }
        }
